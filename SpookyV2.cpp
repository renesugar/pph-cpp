...
void SpookyHash::Update(const void *message, size_t length) {
  ...
  if (m_remainder) {
    ...
    u.p8 = (static_cast<const uint8_t *>(message)) + prefix;
    length -= prefix;
  } else {
    u.p8 = static_cast<const uint8_t *>(message);
  }
...
void SpookyHash::Final(uint64_t *hash1, uint64_t *hash2) {
  ...
  const uint64_t *data = static_cast<const uint64_t *>(m_data);
  ...
  memset(&(reinterpret_cast<uint8_t *>(const_cast<uint64_t *>(data)))[remainder], 0, (sc_blockSize-remainder));

  (reinterpret_cast<uint8_t *>(const_cast<uint64_t *>(data)))[sc_blockSize-1] = remainder;
