...
public:
    ...
    explicit str(const bytearray &b);
...
public:
    PYBIND11_OBJECT(bytearray, object, PYBIND11_BYTEARRAY_CHECK)

    // Allow implicit conversion:
    bytearray(const char *c = "")
        : object(PYBIND11_BYTEARRAY_FROM_STRING_AND_SIZE(c, (ssize_t) strlen(c)), stolen_t{}) {
        if (!m_ptr) pybind11_fail("Could not allocate bytearray object!");
    }

    bytearray(const char *c, size_t n)
        : object(PYBIND11_BYTEARRAY_FROM_STRING_AND_SIZE(c, (ssize_t) n), stolen_t{}) {
        if (!m_ptr) pybind11_fail("Could not allocate bytearray object!");
    }
    ...
    explicit bytearray(const pybind11::str &s);

    operator std::string() const {
        char *buffer;
        ssize_t length;
        length = PYBIND11_BYTEARRAY_SIZE(m_ptr);
        buffer = PYBIND11_BYTEARRAY_AS_STRING(m_ptr);
        if (nullptr == buffer)
            pybind11_fail("Unable to extract bytearray contents!");
        return std::string(buffer, (size_t) length);
    }
};
// Note: breathe >= 4.17.0 will fail to build docs if the below two constructors
...
inline bytearray::bytearray(const pybind11::str &s) {
    object temp = s;
    if (PyUnicode_Check(s.ptr())) {
        temp = reinterpret_steal<object>(PyUnicode_AsUTF8String(s.ptr()));
        if (!temp)
    ...
    char *buffer;
    ssize_t length;
    length = PYBIND11_BYTEARRAY_SIZE(temp.ptr());
    buffer = PYBIND11_BYTEARRAY_AS_STRING(temp.ptr());
    if (nullptr == buffer)
        pybind11_fail("Unable to extract string contents! (invalid type)");
    auto obj = reinterpret_steal<object>(PYBIND11_BYTEARRAY_FROM_STRING_AND_SIZE(buffer, length));
    if (!obj)
        pybind11_fail("Could not allocate bytearray object!");
    m_ptr = obj.release().ptr();
}

inline str::str(const bytearray& b) {
    char *buffer;
    ssize_t length;
    length = PYBIND11_BYTEARRAY_SIZE(b.ptr());
    buffer = PYBIND11_BYTEARRAY_AS_STRING(b.ptr());    
    if (nullptr == buffer)
        pybind11_fail("Unable to extract bytearray contents!");
    auto obj = reinterpret_steal<object>(PyUnicode_FromStringAndSize(buffer, (ssize_t) length));
    if (!obj)
